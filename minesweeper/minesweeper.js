...
function startGame() { // function creating the board and supporting the other functions of the game
    ...
    for (var m = 0; m < mines.length; m++) {
        mines[m].addEventListener("click", soundBoom);
    } // add the event listener to each div in the array

    var pop = document.querySelectorAll("div.hidden:not(.mine)"); // get all the hidden divs EXCEPT mines
    for (var p = 0; p < pop.length; p++) {
        pop[p].addEventListener("click", soundPop);
    } // add the event listener to each div in the array

    // var marked = document.querySelectorAll("div.marked"); // get all the marked divs
    // console.log(marked);
    // for (var m =0; m < marked.length; m++){
    // marked[m].addEventListener("contextmenu", soundMarked);}  // add the event listener to each div in the array
